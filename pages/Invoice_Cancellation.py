...
def show():
    ...
    st.title("📄 Invoice Cancellation Upload")
    
    # Uploader එක Clear කිරීම සඳහා Session State Key එක
    if "inv_uploader_key" not in st.session_state:
        st.session_state["inv_uploader_key"] = 0
    ...
    # අවුරුදු සහ මාස එකට එකතු කර ලැයිස්තුවක් සෑදීම (උදා: 2024 - January)
    month_year_options = []
    ...
    # තෝරාගත් අගයෙන් Year එක සහ Month එක වෙන් කරගැනීම
    ...
    with st.spinner("Checking for existing records..."):
        try:
            ws = sheet.worksheet("invoice_cancellation")
            ...
            if len(all_values) > 1:
                ...
                # තෝරාගත් Year සහ Month එකට අදාළ දත්ත පමණක් වෙන් කරගැනීම
    ...
    if not df_existing.empty:
        ...
        with col_ex2:
            ...
            if st.button("🗑️ Delete Data", use_container_width=True):
                st.session_state["confirm_inv_delete"] = True
                
        # මකා දැමීම තහවුරු කිරීම (Confirmation)
        if st.session_state.get("confirm_inv_delete", False):
                ...
                if st.button("✅ Yes, Delete", use_container_width=True):
                    with st.spinner("Deleting Data..."):
                        try:
                            ...
                            st.session_state["confirm_inv_delete"] = False
    ...
    uploaded_file = st.file_uploader(
        "Upload CSV File (.csv)", 
        type=["csv"], 
        key=f"uploader_{st.session_state['inv_uploader_key']}"
    )
    ...
    if uploaded_file is not None:
        with st.spinner("Reading and Filtering CSV Data..."):
            try:
                ...
                expected_cols = [
                    "Reference Number", "Invoice No", "Reason Code", "Reason", 
                ...
                if st.button("💾 Save to Google Sheets", type="primary"):
                    with st.spinner("Saving data to Google Sheets..."):
                        ...
                        try:
                            ...
                            ws = sheet.add_worksheet(title="invoice_cancellation", rows="1000", cols="20")
                            ws.append_row(["Year", "Month"] + df.columns.tolist())
