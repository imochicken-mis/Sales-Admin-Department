...
def show():
    ...
    st.markdown("""
        ...
        div[data-testid="stSelectbox"] label p {
            font-family: 'Arial', sans-serif !important; font-weight: 800 !important; font-size: 15px !important; color: #03045E !important;
        }
    ...
    st.markdown("<h2 style='text-align: center; color: #03045E; font-weight: 800;'>🔄 Sales Return Data Entry</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #0077B6; font-weight: 600;'>Upload CSV/Excel or Manually enter the Sales Return amounts for each route & rep.</p>", unsafe_allow_html=True)
    st.write("")
    ...
    def load_existing_returns(year, month):
        try:
            ws = sheet2.worksheet("Sales_Return")
            df = pd.DataFrame(ws.get_all_records(default_blank=""))
            if not df.empty:
            ...
            sheet2.add_worksheet(title="Sales_Return", rows=2000, cols=10)
            return pd.DataFrame()
        except Exception:
            return pd.DataFrame()

    def clear_caches():
        load_existing_returns.clear()
    ...
    existing_data_df = load_existing_returns(selected_year, selected_month)
    ...
    # UI Logic for existing data and delete button
    has_data = not existing_data_df.empty

    c1, c2 = st.columns([8, 2], vertical_alignment="bottom")
    with c1:
        if has_data:
            st.success(f"✅ Existing Sales Return data loaded for **{selected_month_year}**.")
        else:
            st.info(f"ℹ️ No existing data found for **{selected_month_year}**. You can enter new data.")
    with c2:
        if st.button("🗑️ Delete Data", use_container_width=True, disabled=not has_data, key="del_btn_sales"):
            st.session_state["confirm_del_sale_rtn"] = True
            
    if st.session_state.get("confirm_del_sale_rtn") and has_data:
        ...
        with dc1:
            ...
            if st.button("✅ Yes, Delete", use_container_width=True):
                with st.spinner("Deleting..."):
                    ...
                    st.session_state["confirm_del_sale_rtn"] = False
                    clear_caches()
                    st.rerun()
        with dc2:
    ...
    if has_data:
        ...
        base_df["Return Amount"] = base_df["Route - Rep Name"].map(existing_dict).fillna(0.0)
    ...
    if uploaded_file is not None:
        try:
            ...
            if "Route - Rep Name" in up_df.columns and "Return Amount" in up_df.columns:
                up_df["Return Amount"] = pd.to_numeric(up_df["Return Amount"].astype(str).str.replace(',', ''), errors='coerce').fillna(0.0)
                upload_dict = dict(zip(up_df["Route - Rep Name"], up_df["Return Amount"]))
                base_df["Return Amount"] = base_df["Route - Rep Name"].map(upload_dict).fillna(0.0)
                st.success("✅ File loaded successfully! Check the table below and click 'Save to Database'.")
            else:
                st.error("⚠️ The uploaded file must contain 'Route - Rep Name' and 'Return Amount' columns.")
        except Exception as e:
            st.error(f"Error reading file: {e}")

    st.write("The calculated Return Amounts per Rep are shown below.")
    ...
    if hasattr(base_df.style, 'map'):
        styled_df = base_df.style.map(highlight_returns, subset=["Return Amount"])
    else:
        styled_df = base_df.style.applymap(highlight_returns, subset=["Return Amount"])
    ...
    st.write("")
    if st.button("💾 Save Returns to Database", type="primary", use_container_width=True):
        with st.spinner("Saving Data to Google Sheets..."):
            try:
                ...
                msg_placeholder.success(f"✅ Sales Return Data for {selected_month_year} saved successfully!")
                time.sleep(2)
