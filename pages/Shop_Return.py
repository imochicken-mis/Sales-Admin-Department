...
def show():
    ...
    st.markdown("<h2 style='text-align: center; color: #03045E; font-weight: 800;'>🏪 Shop Return Data Upload</h2>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #0077B6; font-weight: 600;'>Upload system generated Excel/CSV. Automatically maps Sales Agent names to Reps.</p>", unsafe_allow_html=True)
    st.write("")
    ...
    def load_existing_shop_returns(year, month):
        try:
            ws = sheet2.worksheet("Shop_Return")
            df = pd.DataFrame(ws.get_all_records(default_blank=""))
            if not df.empty:
                filtered = df[(df["Year"].astype(str) == str(year)) & (df["Month"].astype(str) == str(month))]
                if not filtered.empty:
                    return filtered[["Route - Rep Name", "Return Amount"]]
            return pd.DataFrame()
        except gspread.exceptions.WorksheetNotFound:
            sheet2.add_worksheet(title="Shop_Return", rows=2000, cols=10)
            return pd.DataFrame()
        except Exception:
            return pd.DataFrame()

    def clear_caches():
        load_existing_shop_returns.clear()
    ...
    existing_data_df = load_existing_shop_returns(selected_year, selected_month)
    ...
    if not master_reps_df.empty:
        ...
        base_df["Return Amount"] = 0.0
    else:
        st.warning("⚠️ No valid data found in 'MasterData_adjust' tab.")
        base_df = pd.DataFrame(columns=["Route - Rep Name", "Return Amount"])
    ...
    with c1:
        if has_data:
            st.success(f"✅ Existing Shop Return data loaded for **{selected_month_year}**.")
        else:
            st.info(f"ℹ️ No existing data found for **{selected_month_year}**. Please upload a file.")
    with c2:
        if st.button("🗑️ Delete Data", use_container_width=True, disabled=not has_data, key="del_btn_shop"):
            st.session_state["confirm_del_shop_rtn"] = True
            
    if st.session_state.get("confirm_del_shop_rtn") and has_data:
        ...
        with dc1:
            ...
            if st.button("✅ Yes, Delete", use_container_width=True):
                with st.spinner("Deleting..."):
                    ...
                    st.session_state["confirm_del_shop_rtn"] = False
                    clear_caches()
                    st.rerun()
        with dc2:
    ...
    if has_data:
        existing_dict = dict(zip(existing_data_df["Route - Rep Name"], existing_data_df["Return Amount"]))
        display_df["Return Amount"] = display_df["Route - Rep Name"].map(existing_dict).fillna(0.0)

    section_banner("1. Upload Raw Export Data")
    uploaded_file = st.file_uploader("Upload Excel or CSV (Must contain 'Sales Agent' and 'Qty' columns)", type=["csv", "xlsx"])
    ...
    if uploaded_file is not None:
        try:
            ...
            if "Sales Agent" in up_df.columns and "Qty" in up_df.columns:
                up_df["Qty"] = pd.to_numeric(up_df["Qty"].astype(str).str.replace(',', ''), errors='coerce').fillna(0.0)
                with st.spinner("Mapping Agent Names to Routes..."):
                    ...
                    grouped_df = up_df.groupby("Mapped_Rep")["Qty"].sum().reset_index()
                    grouped_df.rename(columns={"Mapped_Rep": "Route - Rep Name", "Qty": "Return Amount"}, inplace=True)
                    
                    upload_dict = dict(zip(grouped_df["Route - Rep Name"], grouped_df["Return Amount"]))
                    display_df["Return Amount"] = display_df["Route - Rep Name"].map(upload_dict).fillna(0.0)
                    ...
                    is_uploaded = True
                    st.success("✅ File processed successfully! Data has been matched with Master Data.")
            else:
                st.error("⚠️ The uploaded file must contain 'Sales Agent' and 'Qty' columns.")
        except Exception as e:
            st.error(f"Error reading file: {e}")

    if not display_df.empty:
        st.markdown("### 2. Review Data")
        st.write("The calculated Shop Return amounts are shown below.")
        ...
        if hasattr(display_df.style, 'map'):
            styled_df = display_df.style.map(highlight_returns, subset=["Return Amount"])
        else:
            styled_df = display_df.style.applymap(highlight_returns, subset=["Return Amount"])

        styled_df = styled_df.format({"Return Amount": "{:.2f}"})
        ...
        # STREAMLIT_CHUNK:Handling Database Save...
        if st.button("💾 Save Shop Returns to Database", type="primary", use_container_width=True):
            with st.spinner("Saving Data to Google Sheets..."):
                try:
                    ws = sheet2.worksheet("Shop_Return")
                    all_records = ws.get_all_records(default_blank="")
                    full_db_df = pd.DataFrame(all_records)
                    
                    if full_db_df.empty:
                        full_db_df = pd.DataFrame(columns=["Year", "Month", "Route - Rep Name", "Return Amount"])
                    
                    # Remove existing records for the selected Year & Month
                    ...
                    # 0 අගයන් ඇති ඒවා අයින් කිරීම (අවශ්‍ය නම් පමණි, දැනට තියාගමු)
                    ...
                    msg_placeholder.success(f"✅ Shop Return Data for {selected_month_year} saved successfully!")
                    time.sleep(2)
